...
add_action('wp_ajax_sf_get_custom_content', 'sf_get_custom_content_fn');
...
function sf_get_custom_content_fn() {
    ...
    if ( isset ( $_POST['id'] ) && isset ( $_POST['settings'] )) { // deprecated
        ...
        if ( is_array( $menu_items ) ) {
            foreach ( $menu_items as $id => $settings ) {
                $result = update_post_meta( $id, SF_ITEM_META, $settings );
                $response[ $id ] = $result;
            }
        }
...
add_action( 'wp_head', 'sf_dynamic', 10 );
add_action( 'wp_head', 'sf_main_html_template', 10 );

add_action( 'admin_post_sf_export_settings', 'sf_export_settings' );
add_action( 'admin_post_sf_import_settings', 'sf_import_settings' );

function sf_export_settings () {
...
function sf_import_settings () {
    ...
    foreach( $import as $setting => &$value ) {
        ...
        if ( $setting === 'sf_copy' || $setting === 'sf_above_logo' || $setting === 'sf_under_logo' || !is_string( $value ) ) { // these can contain HTML
            ...
            if ( isset( $sf_options[ $setting ] ) ) $sf_options[ $setting ] = $value;
        }
    }
...
function sf_main_html_template()
{
    ...
    if ( isset( $sf_show ) && $sf_show && isset( $current_page_sf_menu ) && exclude_builders() ) {
        ob_start();
...
function sf_scripts()
{
    ...
    if ( $sf_show && isset( $current_page_sf_menu ) && exclude_builders()  ) {
